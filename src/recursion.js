...
var sum = function(array) {
  var arrayTemp = array.slice();
  if (!arrayTemp.length) {
    return 0;
  }
  return (arrayTemp.pop() + sum(arrayTemp));
};
...
var reverse = function(string) {
  var reversedString = '';
  if (string.length === 1) {
    return reversedString += string[0];
  }
  reversedString += string[string.length - 1];
  var newString = string.slice(0, -1);
  return reversedString += reverse(newString);
};
...
var countValuesInObj = function(obj, value) {
  ...
  for (var key in obj) {
    runningTotal += countValuesInObj(obj[key], value);
  }
  return runningTotal;
};
